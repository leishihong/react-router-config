...
import { fetchTradeAdd, fetchTradeEdit } from '@/api/vehicle'
import {
  ToggleSelect,
  statusSelect,
  salesStates,
...
export class AddPlatformModal extends PureComponent {
  static propTypes = {
  ...
  handelModal = e => {
    this.props.form.validateFieldsAndScroll(async (err, fieldsValue) => {
      if (!err) {
        ...
        if (this.props.editInfo) {
          const { code } = await fetchTradeEdit(params)
          if (code) {
...
export default Form.create({})(AddPlatformModal)
